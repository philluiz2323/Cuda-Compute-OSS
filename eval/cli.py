...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    p.add_argument("--fill", choices=("random", "lowrank", "decaying-spectrum", "iota"),
                   default="random",
                   help="matrix content. 'random'=hard/full-rank (default), "
                        "'lowrank'=the strategy's happy path, "
                        "'decaying-spectrum'=smoothly decaying singular values")
    ...
    p.add_argument("--seed", type=int, default=None,
                   help="RNG seed. Omit for a fresh unpredictable seed each run "
                        "(recommended -- this is what scoring uses); pass a value "
                        "only to reproduce a specific prior run.")
